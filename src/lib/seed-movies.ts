import { MovieSource, type PrismaClient } from "@/generated/prisma/client";
import { fetchTmdbGenreMap, searchTmdbMovie } from "@/lib/tmdb";

export const mockMovies: Array<{
  title: string;
  year: number;
  synopsis: string;
  runtimeMin: number;
...
export type SeedMovieCatalogResult = {
  householdId: string;
  total: number;
  tmdbMatches: number;
...
/**
 * Crée (si besoin) le foyer par défaut et upsert les 15 films mock, en
 * enrichissant affiche + genre via TMDB si TMDB_API_KEY est disponible.
 * Utilisé par `prisma/seed.ts` (CLI) et par la route `/api/admin/seed`
 * (déclenchement depuis le navigateur, sans terminal).
 */
export async function seedMovieCatalog(
  prisma: PrismaClient
): Promise<SeedMovieCatalogResult> {
